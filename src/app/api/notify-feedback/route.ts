...
export async function POST(req: NextRequest) {
  ...
  const { restaurantName, managerEmail, stars, categories, feedback, contactName, wantsContact, positive } = await req.json()
  ...
  const html = positive
    ? `
      <div style="font-family:sans-serif;max-width:480px;margin:auto">
        <h2 style="color:#16a34a">⭐ Nueva reseña positiva</h2>
        <p><strong>Restaurante:</strong> ${esc(restaurantName)}</p>
        <p><strong>Calificación:</strong> ${starsText} (${stars}/5)</p>
        <p>El cliente eligió dejar una reseña en una plataforma pública.</p>
  ...
  const subject = positive
    ? `⭐ Reseña positiva en ${esc(restaurantName)} — ${starsText}`
    : `⚠️ Feedback negativo en ${esc(restaurantName)} — ${starsText}`
