...
export async function POST(req: Request) {
  ...
  const path = `${businessId}/${priceItemId}/${Date.now()}_${file.name}`
  ...
  try {
    ...
    const msg = await anthropic.messages.create({
      ...
      messages: [{
        ...
        content: [
          ...
          {
            type: 'text',
            text: 'Extrae todo el texto relevante de este documento de producto. Incluye especificaciones, características, precios si los hay, y cualquier información útil para un agente de ventas. Sé exhaustivo.',
          },
        ],
      }],
    })
    ...
    contentText = `[Extracción automática falló — agrega el contenido manualmente]\n\nArchivo: ${file.name}`
  }

  // Save document record
  const { data, error } = await supabaseAdmin().from('product_documents').insert({
