...
async function extractPdfText(buffer: Buffer, filename: string, prompt: string): Promise<string> {
  ...
  try {
    ...
    const msg = await Promise.race([extraction, timeout])
    return msg.content[0].type === 'text' ? msg.content[0].text : ''
  } catch {
    return `[Extracción automática falló]\n\nArchivo: ${filename}`
  }
}
...
export async function POST(req: Request) {
  ...
  const path = `${businessId}/${pathKey}/${Date.now()}_${safeName(file.name)}`
  ...
  if (isText) {
    ...
    contentText = await extractPdfText(buffer, file.name, prompt)
  }
